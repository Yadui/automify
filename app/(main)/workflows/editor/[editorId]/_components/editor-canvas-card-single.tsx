...
const EditorCanvasCardSingle = ({
  ...
  const onRename = () => {
    if (!nodeId) return;
    nodeActions.updateNodeData(nodeId, { title: newName });
    setIsRenameOpen(false);
  };

  const onAddNote = () => {
    if (!nodeId) return;
    nodeActions.updateNodeData(nodeId, {
      metadata: { ...data.metadata, note: newNote },
    });
    setIsNoteOpen(false);
  };
  ...
  const onDuplicate = () => {
    if (!nodeId) return;
    nodeActions.duplicateNode(nodeId);
  };
  ...
  const onPaste = () => {
    nodeActions.pasteNode({ x: xPos + 50, y: yPos + 50 });
  };
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-48">
                ...
                <DropdownMenuItem
                  ...
                  onClick={() => {
                    if (nodeId) nodeActions.deleteNode(nodeId);
                  }}
                >
